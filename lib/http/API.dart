...
class API {
  ...
  void getTodayPlay(RequestCallBack requestCallBack) async {
    ...
    List<Subject> list = resultList.map<Subject>((item) => Subject.fromJson(item)).toList();
  ...
  void getHotComingSoon(RequestCallBack requestCallBack) async {
    ...
    var resultList = result['subjects'];
    List<Subject> hots = resultList.map<Subject>((item) => Subject.fromJson(item)).toList();

    //即将上映
    result = await _request.get(COMING_SOON + '?apikey=0b2bdeda43b5688921839c8ecb20399b');
    resultList = result['subjects'];
    List<Subject> comingSoons = resultList.map<Subject>((item) => Subject.fromJson(item)).toList();
  ...
  void getWeekly(RequestCallBack requestCallBack) async {
    ...
    var resultList = result['subjects'];
    List<SubjectEntity> list = resultList.map<SubjectEntity>((item) => SubjectEntity.fromJson(item)).toList();
    requestCallBack(list);
  }
  ...
  void getMovieReviews(String id, RequestCallBack requestCallBack) async {
    final result = await _request.get('/v2/movie/subject/$id/reviews?apikey=0b2bdeda43b5688921839c8ecb20399b');
